...
export async function restoreFiles(files, apiFn, options = {}) {
    ...
    for (const group of groups) {
        ...
        let answer;
        if (autoApprove || prefixApproved || exactAnswer === 'y' || exactAnswer === 'a') {
            answer = 'y';
        } else if (exactAnswer === 'n') {
            answer = 'n';
        } else {
            answer = await promptFn(group.directory, [...group.files, ...group.folders]);
        ...
        for (const folder of group.folders) {
            try {
                await apiFn('/2/files/create_folder_v2', {path: folder.path, autorename: false});
                foldersCreated++;
            } catch (err) {
