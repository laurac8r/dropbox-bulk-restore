...
export async function run(options) {
  ...
  if (failedRetries.length > 0) {
    ...
    if (retryable.length < failedRetries.length) {
      info(
        `Skipping ${failedRetries.length - retryable.length} permanent failures from retry queue`,
      );
    }
  ...
  while (hasMore) {
    const pageResult = await discoverDeletedPage(path, api, {
      cursor,
      filterPrefix,
    });
    ...
    page++;
    spinner.update(
      `Scanning page ${page}: ${totalDiscovered} found, ${totalResolved} restorable...`,
    );

    if (pageResult.deleted.length > 0) {
      // Resolve revisions for this page
      const { resolved: pageResolved, errors: pageErrors } =
        await resolveRevisions(pageResult.deleted, api, {
          concurrency,
          retentionDays,
          logFn: (path, response) =>
            debug(
              `  DEBUG list_revisions ${path}: ${JSON.stringify(response)}`,
            ),
        });
      ...
      let toRestore = tracker
        ? tracker.filterUnprocessed(pageResolved)
        : pageResolved;
      ...
      if (toRestore.length > 0) {
        ...
        const pageResult = await restoreFiles(toRestore, api, {
          ...
          onProgress: (completed, total) => {
            spinner.update(
              `Restoring ${totalRestored + completed}/${totalAttempted + total}...`,
            );
          },
        });
  ...
  writeFn("\n");
  info(
    `Found ${totalDiscovered} deleted, ${totalResolved} restorable (${allResolveErrors.length} expired/skipped)`,
  );
  ...
  if (retention.length > 0)
    warn(
      `Skipped ${retention.length} expired files (past ${retentionDays}-day retention)`,
    );
  ...
  warn(
    `Restored ${totalRestored}/${totalAttempted} files, ${restoreErrors.length} errors`,
  );
...
Resume:
  ...
  const targetPath =
    pathFlag !== -1 ? args[pathFlag + 1] || defaultPath : defaultPath;
  const limitFlag = args.indexOf("--limit");
  const limit =
    limitFlag !== -1 ? parseInt(args[limitFlag + 1], 10) : undefined;
  const dryRun = args.includes("--dry-run");
  const autoApprove = args.includes("--yes") || args.includes("-y");
  const logLevelFlag = args.indexOf("--log-level");
  const logLevel =
    logLevelFlag !== -1
      ? (args[logLevelFlag + 1] || "INFO").toUpperCase()
      : "INFO";
