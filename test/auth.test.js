...
describe("runPKCEFlow", () => {
  test("starts server, opens auth URL, and exchanges code for tokens", async () => {
    ...
    await fetch(
      `${redirectUri}?code=auth-code-123&state=${encodeURIComponent(state)}`,
    );
  ...
  test("rejects when callback receives no code and no error", async () => {
    ...
    await expect(flowPromise).rejects.toThrow(
      "OAuth authorization failed: no code received",
    );
  });
  ...
  test("rejects when token exchange returns ok: false", async () => {
    ...
    const state = authUrl.searchParams.get("state");
    await fetch(
      `${redirectUri}?code=some-code&state=${encodeURIComponent(state)}`,
    );
  ...
  test("handled flag prevents double token exchange within single flow", async () => {
    ...
    const r1 = fetch(
      `${redirectUri}?code=c1&state=${encodeURIComponent(state)}`,
    );
    const r2 = fetch(
      `${redirectUri}?code=c2&state=${encodeURIComponent(state)}`,
    ).catch(() => null);
...
describe("getToken", () => {
  ...
  test("runs PKCE flow when no tokens exist", async () => {
    ...
    await fetch(
      `${callbackUrl}?code=test-code&state=${encodeURIComponent(pkceState)}`,
    );
  ...
  test("falls through to PKCE when refresh fails", async () => {
    ...
    await new Promise((r) => setTimeout(r, 150));
    await fetch(
      `${callbackUrl}?code=fallback-code&state=${encodeURIComponent(pkceState)}`,
    );
  ...
  test('propagates error whose message incidentally contains "400" as substring (not a real HTTP 400)', async () => {
    ...
    const fakeErr = new Error(
      "upstream returned 14001 results, not a real HTTP 400",
    );
