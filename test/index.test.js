...
describe("run (orchestrator)", () => {
  ...
  test("resolved file listing hidden at INFO, shown at DEBUG", async () => {
    ...
    expect(
      infoLogs.some(
        (m) => m.includes("/pics/photo.jpg") && m.includes("abc123"),
      ),
    ).toBe(false);
  ...
  test("summarizes errors by category instead of listing each one", async () => {
    ...
    expect(
      allLogs.some((m) => /2.*expired/i.test(m) || /2.*retention/i.test(m)),
    ).toBe(true);
    ...
    expect(
      allLogs.some(
        (m) => m.includes("/pics/c.jpg") && m.includes("Some restore error"),
      ),
    ).toBe(true);
  });
  ...
  test("previousErrors with permanent failures are not retried", async () => {
    ...
    const result = await run({
      ...
      previousErrors: [
        {
          path: "/pics/expired.jpg",
          rev: "rev-expired",
          error: "Past retention window (deleted 452 days ago, limit 180 days)",
        },
        {
          path: "/pics/invalid.jpg",
          rev: "rev-invalid",
          error: "Dropbox API error 409: invalid_revision",
        },
        {
          path: "/pics/transient.jpg",
          rev: "rev-transient",
  ...
  describe("logLevel", () => {
    ...
    test("logLevel DEBUG includes raw API responses", async () => {
      ...
      const logs = logFn.mock.calls.map((c) => c[0]);
      expect(
        logs.some((m) => m.includes("DEBUG") && m.includes("list_revisions")),
      ).toBe(true);
    });
  ...
  test("discovers files when target path itself is deleted (path/not_found fallback)", async () => {
    ...
    const mockApi = vi.fn().mockImplementation((endpoint, body) => {
      if (endpoint === "/2/files/list_folder" && body.path === "/pics/album") {
        return Promise.reject(
          new Error("Dropbox API error 409: path/not_found"),
        );
      }
          ...
          entries: [
            {
              ".tag": "deleted",
              name: "a.jpg",
              path_display: "/pics/album/a.jpg",
            },
            {
              ".tag": "deleted",
              name: "other.jpg",
              path_display: "/pics/other.jpg",
            },
          ],
          has_more: false,
        });
  ...
  test("successful retries clear errors.json so they are not retried again", async () => {
    ...
    const errors = JSON.parse(
      fs.readFileSync(path.join(tmpDir, "errors.json"), "utf8"),
    );
